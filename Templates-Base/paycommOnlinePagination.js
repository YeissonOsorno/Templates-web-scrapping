...
(function() {
  ...
  (function() {
    var out = {};
    var next_page_selector = "a.js-pagination-link-next "; //selector to identify the next button
    var stop = 'div[class="col-xs-6 pagination-style-box"]'; //selector to identify the last page
  
    var clickable_elem = document.querySelector(next_page_selector);
    var stop = document.querySelector(stop).textContent.trim().split(' ');
    if(clickable_elem){
      if(stop[1].split('-')[1].trim()== stop[3]){
...
(function() {
    ...
    if(document.querySelector('span[aria-label="Salary Range"]')){
      job.source_salary = document.querySelector('span[aria-label="Salary Range"]').textContent.trim();
    }
  
    job.html      = full_html.innerHTML.trim();    
    job.html = removeTextBefore(job.html, 'Top 5 Duties:', false);
